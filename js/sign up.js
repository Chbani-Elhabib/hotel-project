...
$(document).ready(function () {
  ...
  login__input.each((e) => {
    login__input[e].addEventListener("keyup", (el) => {
      ...
      if (
        ...
        ( validation.eq(3).attr("class") == "validation icon-check" && login__input[2].value != login__input[3].value ) ||
        (!login__input[3].value.match(validation_password) && validation.eq(3).attr("class") == "validation icon-check")
      ) {
  ...
  login__input[3].addEventListener("blur", el => {
      // validation confirm password
      if (login__input[2].value != login__input[3].value || !login__input[3].value.match(validation_password)) {
        validation.eq(3).removeClass("icon-check");
        validation.eq(3).addClass("icon-cancel-circle");
        login__input.eq(3).css("border-bottom-color", "rgb(245, 1, 57)");
      }
  ...
  login__submit[0].addEventListener("click", e => {
    ...
    if(login__input[0].value.length == 0){
      ...
      validation.eq(0).addClass("icon-cancel-circle");
      login__input.eq(0).css("border-bottom-color", "rgb(245, 1, 57)");
    }else if(!login__input[0].value.match(validation_Username) || login__input[0].value.length < 4){
      e.preventDefault();
      Swal.fire({
        icon: 'error',
